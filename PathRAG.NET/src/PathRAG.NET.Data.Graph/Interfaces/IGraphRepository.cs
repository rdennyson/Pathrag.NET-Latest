...
public interface IGraphRepository
{
    ...
    Task<GraphEntity?> GetEntityByNameAsync(string entityName, CancellationToken cancellationToken = default);
    Task<GraphEntity?> GetEntityByIdAsync(Guid id, CancellationToken cancellationToken = default);
    Task<IEnumerable<GraphEntity>> GetAllEntitiesAsync(int limit = 100, CancellationToken cancellationToken = default);
    Task<IEnumerable<GraphEntity>> GetEntitiesByNamesAsync(IEnumerable<string> entityNames, CancellationToken cancellationToken = default);
    ...
    Task<GraphRelationship?> GetRelationshipAsync(string sourceEntity, string targetEntity, CancellationToken cancellationToken = default);
    Task<GraphRelationship?> GetRelationshipByIdAsync(Guid id, CancellationToken cancellationToken = default);
    Task<IEnumerable<GraphRelationship>> GetAllRelationshipsAsync(int limit = 100, CancellationToken cancellationToken = default);
    ...
    Task<IEnumerable<(GraphEntity Source, GraphRelationship Edge, GraphEntity Target)>> GetOneHopPathsAsync(string entityName, CancellationToken cancellationToken = default);
    Task<IEnumerable<(List<GraphEntity> Nodes, List<GraphRelationship> Edges)>> GetMultiHopPathsAsync(string sourceEntity, int maxHops = 3, CancellationToken cancellationToken = default);
